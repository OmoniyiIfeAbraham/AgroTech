...
router.post('/:id', async(req, res, next) => {
    ...
    try {
                ...
                if(productContent.length > 30) {
                    ...
                    if(productImage != null) {
                        if(productImage.mimetype=='image/apng' || productImage.mimetype=='image/avif' || productImage.mimetype=='image/gif' || productImage.mimetype=='image/jpeg' || productImage.mimetype=='image/png' || productImage.mimetype=='image/svg+xml' || productImage.mimetype=='image/webp') {
                            ...
                            console.log(result)
                            if(result !== "ok") {
                                res.render('admin/products/updateProduct', { product, msg: 'Unable to Update Succesfully!!!' })
                            } else {
                                async function Cloud() {
                                    ...
                                    productsMod.findByIdAndUpdate({ _id: id }, { name: productName, image: upload.secure_url, imagePublicId: upload.public_id, price: productPrice, content: productContent, snippet: productContent.slice(0, 30), category: productCategory}, (err, docs) => {
                                        if (err) {
                                            console.log(err)
                                            next(err)
                                        } else {
                            ...
                            res.render('admin/products/updateProduct', { product, msg: 'Invalid Image File type!!!' })
                        }
                    } else {
                        productsMod.findByIdAndUpdate({ _id: id }, { name: productName, price: productPrice, content: productContent, snippet: productContent.slice(0, 30), category: productCategory}, (err, docs) => {
                            if (err) {
                                console.log(err)
                                next(err)
                            } else {
                    ...
                    res.render('admin/products/updateProduct', { product, msg: 'Product Content Description length must be Greater than Thirty(30) Characters' })
                }
            } else {
                res.render('admin/products/updateProduct', { product, msg: 'Please fill all fields' })
            }
        } else {
            res.redirect('/login')
        }
        ...
        res.render('admin/products/updateProduct', { product, msg: 'An Error Ocurred!!!' })
        next(error)
    }
})

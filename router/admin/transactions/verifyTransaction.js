      ...
      if (buy.amount == amount / 100 && product.price == amount / 100) {
          ...
          (err, docs) => {
            if (err) {
              ...
              if (product.category === "livestock product") {
                res.render("client/products/livestockProducts", {
                  products,
                  msg: "",
                  resp: `Your Payment Was Succesful and a mail with Details has been sent to this Address: ${buy.email}.`,
                });
              } else {
                res.render("client/products/agriculturalProducts", {
                  products,
                  msg: "",
                  resp: `Your Payment Was Succesful and a mail with Details has been sent to this Address: ${buy.email}.`,
                });
        ...
        if (product.category === "livestock product") {
          res.render("client/products/livestockProducts", {
            products,
            msg: "",
            resp: `Your Payment Failed due to Incorrect Price. Details has been sent to this Address: ${buy.email}.`,
          });
        } else {
          res.render("client/products/agriculturalProducts", {
            products,
